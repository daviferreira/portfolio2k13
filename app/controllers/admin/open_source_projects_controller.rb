class Admin::OpenSourceProjectsController < Admin::AdminController
  ...
  def create
    ...
    if @open_source_project.save
      redirect_to admin_open_source_projects_path,
                  :flash => { :success => t("open_source_projects.created") }
    else
      render "new"
    end
  end
  ...
  def update
    ...
    if @open_source_project.update_attributes(params[:open_source_project])
      redirect_to edit_admin_open_source_project_path(@open_source_project),
                  :flash => { :success => t("open_source_projects.updated") }
    else
      render "edit"
    end
  end
  ...
  def destroy
    ...
    redirect_to admin_open_source_projects_path,
                :flash => { :notice => t("open_source_projects.deleted") }
  end
